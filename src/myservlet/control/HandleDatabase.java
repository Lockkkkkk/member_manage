...
public class HandleDatabase extends HttpServlet {
	...
	public void continueDoPost(HttpServletRequest request, HttpServletResponse response)
		...
		showBean.setPageSize(5); // 每页显示3条记录
	...
	public StringBuffer show(int page, int pageSize, CachedRowSetImpl rowSet) {
		...
		try {
			...
			for (int i = 1; i <= pageSize; i++) {
				...
				str.append("<form action='helpdelete' method=post>");
				str.append("<tr>");
				...
				str.append("<td>" + "<input type=submit value='delete'>" + "<input type=hidden name='username' value='"
						+ name + "'>");
				str.append("</form>");
				str.append("<form action='change1.jsp' method=post>");
				str.append("<input type=submit value='change'>"+"<input type=hidden name='username' value='"+name+"'>");
				str.append("</td>");
				str.append("</form>");
	...
	public void continueDoGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (rs.next()) {
				...
				inform.setNichen(rs.getString(4));
				inform.setSex(rs.getString(5));
				inform.setPhone(rs.getString(6));
				inform.setBirthday(rs.getString(7));
				inform.setAddress(rs.getString(8));
